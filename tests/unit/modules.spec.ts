...
describe('modules', () => {
  describe('module registration', () => {
    it('dynamic module registration', () => {
      const store = new Vuex.Store({
        ...
        modules: {
          foo: {
            state: { bar: 1 },
            mutations: { inc: (state: any) => state.bar++ },
            actions: { incFoo: ({ commit }: any) => commit('inc') },
            getters: { bar: (state: any) => state.bar },
          },
        },
      })
      expect(() => {
        store.registerModule('hi', {
          state: { a: 1 },
          mutations: { inc: (state: any) => state.a++ },
          actions: { inc: ({ commit }: any) => commit('inc') },
          getters: { a: (state: any) => state.a },
        })
      }).not.toThrow()
    ...
    it('dynamic module registration preserving hydration', () => {
      ...
      store.registerModule('a', {
        namespaced: true,
        getters: { foo: (state: any) => state.foo },
        actions: { foo: actionSpy },
        mutations: { foo: mutationSpy },
      }, { preserveState: true })
    ...
    it('should keep getters when component gets destroyed', async () => {
      ...
      const CompA = {
        template: `<div />`,
        created() {
          (this as any).$store.registerModule('moduleA', moduleA)
        },
      }
      ...
      const vm = mount(store, {
        ...
        data: () => ({ show: 'a' }),
        render() {
          return (this as any).show === 'a' ? h(CompA) : h(CompB)
        },
      })
  ...
  // #524
  it('should not fire an unrelated watcher', async () => {
    const spy = vi.fn()
    const store = new Vuex.Store({
      modules: {
        a: {
  ...
  describe('modules usage', () => {
    ...
    it('module: action', () => {
      let calls = 0
      const makeAction = (n: any) => {
        return {
          [TEST]({ state, rootState }: any) {
            calls++
    ...
    it('module: nested namespace', () => {
      ...
      const actionSpys: any[] = []
      const mutationSpys: any[] = []
      const createModule = (name: any, namespaced: any, children?: any) => {
        const actionSpy = vi.fn()
        const mutationSpy = vi.fn()
      ...
      const store = new Vuex.Store({
        ...
        modules: {
          a: {
            ...
            getters: {
              foo: (state: any) => state.value,
              bar: (state: any, getters: any) => getters.foo,
              baz: (state: any, getters: any, rootState: any, rootGetters: any) => rootGetters.foo,
            },
          },
        },
      })
    ...
    it('module: action context is namespaced in namespaced module', async () => {
      ...
      const store = new Vuex.Store({
        ...
        modules: {
          a: {
            ...
            actions: {
              foo: moduleActionSpy,
              test({ dispatch, commit, getters, rootGetters }: any) {
      ...
      await store.dispatch('a/test')
    })

    it('module: use other module that has same namespace', async () => {
      const actionSpy = vi.fn()
      const mutationSpy = vi.fn()
        ...
        modules: {
          parent: {
            ...
            modules: {
              ...
              b: {
                state: { value: 'b' },
                getters: { bar: (state: any, getters: any) => getters.foo },
                actions: {
                  test({ dispatch, commit, getters }: any) {
      ...
      await store.dispatch('parent/test')
    })

    // Skip warn tests for now unless we implement exact warnings
    /*
    ...
    */
    ...
    it('dispatching multiple actions in different modules', async () => {
      const store = new Vuex.Store({
        modules: {
          a: {
            actions: {
    ...
    it('root actions dispatched in namespaced modules', async () => {
      const store = new Vuex.Store({
        modules: {
          a: {
            namespaced: true,
    ...
    it('plugins', () => {
      ...
      const actionSpy = vi.fn()
      const mutations: any[] = []
      const subscribeActionSpy = vi.fn()
      const store = new Vuex.Store({
        state: {
          a: 1,
        },
        ...
        plugins: [
          (store: any) => {
            ...
            store.subscribe((mut: any, state: any) => {
    ...
    it('action before/after subscribers', async () => {
